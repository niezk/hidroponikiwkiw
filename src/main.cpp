...
#define IN3 19  // Vitamin A (TDS control)
#define IN4 23  // Vitamin B (TDS control)
...
#define VPIN_RELAY3 V5  // Vitamin A
#define VPIN_RELAY4 V6  // Vitamin B

// Threshold Virtual Pins
#define VPIN_TDS_THRESHOLD V7
#define VPIN_TEMP_THRESHOLD V8
...
bool relay3State = false;  // Vitamin A
bool relay4State = false;  // Vitamin B
...
void loop() {
  if (wifiConfigMode) {
    ...
    server.handleClient();  //Handle web requests even when connected
    Blynk.run();
    timer.run();
...
void startConfigPortal() {
  ...
  Serial.println("WiFi: ESP32-Config");
  Serial.println("Pass: tme12345");
  Serial.print("AP IP address: ");
  Serial.println(WiFi.softAPIP());
  
  // Display config info on LCD
  lcd.clear();
  lcd.print(" System OK!");
  lcd.setCursor(0, 1);
  lcd.print("IP:");
  ...
  server.on("/generate_204", handleConfigPage);  // Android captive portal
  server.on("/fwlink", handleConfigPage);        // Microsoft captive portal
  server.onNotFound(handleNotFound);             // Catch all other requests
  server.begin();
}

// Handle all undefined routes - redirect to config page for captive portal
void handleNotFound() {
...
void handleConfigPage() {
  ...
  html += "<input type='text' name='blynk' required placeholder='Your Auth Token'>";
...
void handleStatusPage() {
  ...
  html += "<div class='auto-status'><strong>TDS Auto:</strong> " + String(autoTDS ? "ENABLED" : "DISABLED") + " (Threshold: " + String(tdsThreshold, 1) + " ppm)<br><span class='relay-label'>Controls: Relay 3 & 4 (Vitamins)</span></div>";
  ...
  html += "<div class='relay " + String(relay3State ? "relay-on" : "relay-off") + "'><div><strong>Relay 3 (Vitamin A):</strong><br><span class='relay-label'>TDS Control</span></div><span>" + String(relay3State ? "ON" : "OFF") + "</span></div>";
  html += "<div class='relay " + String(relay4State ? "relay-on" : "relay-off") + "'><div><strong>Relay 4 (Vitamin B):</strong><br><span class='relay-label'>TDS Control</span></div><span>" + String(relay4State ? "ON" : "OFF") + "</span></div>";
...
void connectToWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("- Relay 3 (IN3): Vitamin A (TDS control)");
    Serial.println("- Relay 4 (IN4): Vitamin B (TDS control)");
    ...
    // Setup timer to read sensors every 2 seconds
    timer.setInterval(2000L, readSensors);
    
    // Start web server for status and logout
    ...
    // Clear credentials and restart config
    preferences.begin("wifi-config", false);
    preferences.clear();
    preferences.end();
    ESP.restart();
...
void readSensors() {
  // Read TDS sensor (analog value, convert to ppm)
  ...
  // Read pH sensor (analog value, convert to pH)
  ...
  Serial.print("Temp (DS18B20): "); Serial.print(tempValue); Serial.println(" °C");
...
// Check automation rules and control relays
void checkAutomation() {
  ...
  // TDS Automation - Controls Relay 3 (Vitamin A) and Relay 4 (Vitamin B)
  if (autoTDS) {
    bool shouldActivate = (tdsValue < tdsThreshold);  // Activate if TDS is LOW (needs vitamins)
    
    // Control Relay 3 (Vitamin A)
    if (!relay3Manual) {
      if (relay3State != shouldActivate) {
        relay3State = shouldActivate;
        digitalWrite(IN3, !relay3State);  // Invert for relay module
        Blynk.virtualWrite(VPIN_RELAY3, relay3State);
        Serial.println("AUTO: TDS - Vitamin A (Relay 3) " + String(relay3State ? "ON" : "OFF"));
      }
    }
    
    // Control Relay 4 (Vitamin B)
    if (!relay4Manual) {
      if (relay4State != shouldActivate) {
        ...
        Serial.println("AUTO: TDS - Vitamin B (Relay 4) " + String(relay4State ? "ON" : "OFF"));
      }
    }
  }
  
  // Temperature Automation (if needed, currently no relay assigned)
  // You can add temperature-based control here if needed
}
...
BLYNK_WRITE(VPIN_RELAY1) {
  ...
  relay1Manual = true;  // Mark as manually controlled
...
BLYNK_WRITE(VPIN_RELAY3) {
  ...
  digitalWrite(IN3, !relay3State);
  relay3Manual = true;
  Serial.print("MANUAL: Relay 3 (Vitamin A): ");
...
BLYNK_WRITE(VPIN_RELAY4) {
  ...
  relay4Manual = true;
  Serial.print("MANUAL: Relay 4 (Vitamin B): ");
...
BLYNK_WRITE(VPIN_AUTO_TDS) {
  ...
  preferences.end();
  if (autoTDS) {
    relay3Manual = false;  // Clear manual override when automation is enabled
    relay4Manual = false;
  }
